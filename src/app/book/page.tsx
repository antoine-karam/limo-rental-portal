...
function PaymentForm({ bookingId }: { bookingId: string }) {
  ...
  return (
    <div className={styles.actions}>
      <button className={styles.nextButton} onClick={pay} type="button">
        Pay now
      </button>
    </div>
  );
...
export default function BookPage() {
  ...
  useEffect(() => {
    const initialize = () => {
      if (!window.google?.maps || !pickupInputRef.current || !dropoffInputRef.current) return;
  ...
  useEffect(() => {
    if (!pickup || (!dropoff && rideType !== "HOURLY") || !window.google?.maps) return;
    ...
    if (!dropoff) {
      new google.maps.Marker({ map, position: { lat: pickup.lat, lng: pickup.lng } });
      return;
    }
  ...
  const step1Valid = Boolean(
    pickup &&
      scheduledDate &&
      scheduledTime &&
      (rideType === "HOURLY" ? Number(hours) > 0 : dropoff),
  );
  ...
  return (
    <div className={styles.page}>
      <div className={styles.stepper}>
        {["Ride", "Vehicle", "Passenger", "Payment"].map((label, index) => (
          <span
            key={label}
            className={`${styles.stepPill} ${step === index + 1 ? styles.stepActive : ""}`}
          >
            {label}
          </span>
        ))}
      ...
      <div className={styles.layout}>
        <section className={styles.panel}>
          {step === 1 && (
            <>
              <h2 className={styles.title}>Ride details</h2>
              <div className={styles.cards}>
                {rideTypes.map((type) => (
                  <button
                    key={type.value}
                    className={`${styles.card} ${rideType === type.value ? styles.cardSelected : ""}`}
              ...
              <div className={styles.field}>
                ...
                <input id="pickup" ref={pickupInputRef} placeholder="Enter pickup location" />
              </div>

              {rideType !== "HOURLY" ? (
                <div className={styles.field}>
                  <label htmlFor="dropoff">Dropoff address</label>
                  <input id="dropoff" ref={dropoffInputRef} placeholder="Enter dropoff location" />
                </div>
              ) : (
                <div className={styles.field}>
                  <label htmlFor="duration">Duration (hours)</label>
                  <select id="duration" value={hours} onChange={(event) => setHours(event.target.value)}>
              ...
              {distanceKm > 0 && (
                <div className={styles.banner}>
                  Estimated distance: {distanceKm.toFixed(1)} km · Estimated duration: {durationText}
                </div>
              )}
          ...
          {step === 2 && (
            <>
              <h2 className={styles.title}>Vehicle selection</h2>
              <div className={styles.cards}>
                {vehicles.map((vehicle) => (
                  <button
                    key={vehicle.id}
                    className={`${styles.card} ${selectedVehicle?.id === vehicle.id ? styles.cardSelected : ""}`}
                  ...
                  >
                    ...
                    <p>Amenities: {vehicle.amenities.join(", ") || "Standard amenities"}</p>
                    <p>
        ...
        <aside className={styles.panel}>
          {step === 1 ? (
            ...
            <div className={styles.summary}>
              ...
              <p>Date and time: {summaryTime}</p>
              <p>Estimated price: {selectedVehicle ? `${selectedVehicle.currency} ${selectedVehicle.price.toFixed(2)}` : "-"}</p>
            </div>
          )}
        </aside>
      </div>
